...
import { CAPS_TEACHER_SETTINGS_KEY, DEFAULT_CAPS_TEACHER_SETTINGS, normalizeCapsTeacherSettings, type CapsTeacherSettings } from '@/lib/curriculum/teacher-config';
...
export default function CapsOnboardingPage() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('nahaCapsRevisitContext');
      if (raw) {
        const revisit = JSON.parse(raw) as { grade?: number; subject?: CapsSubject; topicId?: string; goal?: CapsLearningGoal };
        if (GRADES.includes(revisit.grade as Grade)) setGrade(revisit.grade as Grade);
        if (revisit.subject === 'mathematics' || revisit.subject === 'physical-sciences') setSubject(revisit.subject);
        if (revisit.goal === 'learn' || revisit.goal === 'assess' || revisit.goal === 'practice' || revisit.goal === 'simulate') setGoal(revisit.goal);
        if (revisit.topicId) setTopicId(revisit.topicId);
        localStorage.removeItem('nahaCapsRevisitContext');
      }
    } catch {
      // Defaults are safe if a revisit context is unavailable.
    }
  }, []);

  const topics = useMemo(() => CAPS_SOWETO_PILOT.filter((topic) => topic.subjects.includes(subject) && topic.grades.includes(grade)), [grade, subject]);
    ...
    <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6 lg:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-4xl items-center">
        <section className="w-full rounded-3xl border bg-card p-6 shadow-sm sm:p-10">
          <div className="mb-8 flex items-center gap-3"><div className="flex size-11 items-center justify-center rounded-2xl bg-primary/10"><GraduationCap className="size-6 text-primary" /></div><div><p className="text-sm font-medium text-muted-foreground">NahaLabs Online Academy</p><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Start your classroom</h1></div></div>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.15fr_.85fr]">
            ...
            <aside className="rounded-2xl border bg-muted/40 p-5"><div className="mb-5 flex size-10 items-center justify-center rounded-xl bg-background"><Sparkles className="size-5" /></div><p className="mb-1 text-sm font-medium text-muted-foreground">Your classroom</p><h2 className="text-xl font-semibold">Grade {grade} {subject === 'mathematics' ? 'Mathematics' : 'Physical Sciences'}</h2><p className="mt-2 text-sm text-muted-foreground">{selectedTopic?.label ?? 'Choose a topic'}</p><div className="mt-5 space-y-2 text-sm text-muted-foreground"><p>✓ Step-by-step teaching</p><p>✓ Interactive learning</p><p>✓ Practice and formative checks</p><p>✓ Classmate discussion</p></div><Button className="mt-7 w-full" size="lg" onClick={startClass} disabled={!selectedTopic}>Start class<ArrowRight className="ml-2 size-4" /></Button><Button asChild variant="ghost" className="mt-2 w-full"><a href="/progress">View my progress</a></Button></aside>
          </div>
        </section>
      </div>
    </main>
