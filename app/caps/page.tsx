...
const SUBJECTS: Array<{ id: CapsSubject; label: string; description: string }> = [
  {
    id: 'mathematics',
    label: 'Mathematics',
    description: 'Algebra, functions, geometry, trigonometry and data.',
  },
  {
    id: 'physical-sciences',
    label: 'Physical Sciences',
    description: 'Mechanics, waves, electricity, matter and reactions.',
  },
];
...
export default function CapsOnboardingPage() {
  ...
  const topics = useMemo(
    () => CAPS_SOWETO_PILOT.filter((topic) => topic.subject === subject && topic.grades.includes(grade)),
    [grade, subject],
  );
  ...
  const startClass = () => {
    ...
    const requirement = [
      `Grade ${grade} ${subject === 'mathematics' ? 'Mathematics' : 'Physical Sciences'}`,
    ...
    try {
      ...
      localStorage.setItem(
        'nahaCapsLearnerContext',
        JSON.stringify({ grade, subject, topicId: selectedTopic.id, goal, version: 1 }),
      );
    } catch {
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-4xl items-center">
        <section className="w-full rounded-3xl border bg-card p-6 shadow-sm sm:p-10">
          ...
          <p className="mb-8 max-w-2xl text-muted-foreground">
            Choose your grade, subject and goal. We&apos;ll open an interactive classroom with an AI teacher,
            classmates, activities and a whiteboard — starting from the CAPS pilot curriculum.
          </p>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.15fr_.85fr]">
            <div className="space-y-7">
                  ...
                  {GRADES.map((item) => (
                    <button
                      ...
                      className={cn(
                        ...
                        grade === item ? 'border-primary bg-primary/10 text-primary' : 'hover:bg-muted',
                      )}
                    >
                      Grade {item}
                    </button>
                  ...
                  {SUBJECTS.map((item) => (
                    <button
                      ...
                      className={cn(
                        ...
                        subject === item.id ? 'border-primary bg-primary/10' : 'hover:bg-muted',
                      )}
                    >
              ...
              <div>
                ...
                >
                  {topics.length === 0 ? (
                    <option value="">No pilot topic yet</option>
                  ) : (
                    topics.map((topic) => (
                  ...
                  {GOALS.map((item) => (
                    <button
                      ...
                      className={cn(
                        ...
                        goal === item ? 'border-primary bg-primary/10 text-primary' : 'hover:bg-muted',
                      )}
                    >
                      {item}
                    </button>
            ...
            <aside className="rounded-2xl border bg-muted/40 p-5">
              <div className="mb-5 flex size-10 items-center justify-center rounded-xl bg-background">
              ...
              <p className="mt-2 text-sm text-muted-foreground">{selectedTopic?.title ?? 'Choose a topic'}</p>
              <div className="mt-5 space-y-2 text-sm text-muted-foreground">
                <p>✓ Step-by-step teaching</p>
                <p>✓ Interactive learning</p>
                <p>✓ Practice and formative checks</p>
                <p>✓ AI classmates for discussion</p>
              </div>
              <Button className="mt-7 w-full" size="lg" onClick={startClass} disabled={!selectedTopic}>
